...
(async () => {
  ...
  function formatTembakSaldoMk(dataString) {
    ...
    return dataString
      .trim()
      .split("\n")
      .filter((line) => line.trim() !== "List_Tembak_Saldo")
      .map((line, index) => {
  ...
  async function processAndSendDownlineList(
    msg,
    uplineId,
    pinTrx = 1234,
    ...
    manager.query(
      `SELECT idreseller, NAMARESELLER, saldo FROM avr.masterreseller where idupline='${uplineId}'`,
  ...
  client.on("message", async (msg) => {
    ...
    if (msg.type == "e2e_notification") {
      ...
      msg.from == process.env.hpTarik + "@c.us"
    ) {
      ...
      manager.query(
        ...
        function (err, rows) {
          ...
          client.sendMessage(msg.from, formattedListTrx);
        }
      );
      ...
      msg.body.startsWith("Ozzi") &&
      msg.from == hpTarikArifin + "@c.us"
    ) {
      client.sendMessage(msg.from, "Siaaap... Tunggu dilit iseh Loading...");
      ...
      msg.body.startsWith("Downline anda:") &&
      msg.from == hpLDLGp + "@c.us"
    ) {
      ...
      function (err, rows, fields) {
        if (!err) {
          rows.forEach(function (row) {
            ...
            tujuan = "62" + S(tujuan).right(tujuan.length - 1).s;
            tujuan = tujuan + "@c.us";
            if (row.untukuser.match(/^[0-9]*$/)) {
              client.sendMessage(tujuan, row.isi);
            } else {
              console.log("nomor aneh", tujuan);
            }
          });
          state = true;
        } else {
