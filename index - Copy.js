    ...
    return lines.map(line => {
        ...
        if (match) {
            ...
            if (saldo >= 500000) {
                ...
                return `${name} = Rp${saldo.toLocaleString()} =>> https://wa.me/6281369083333?text=${encodedMessage}`;
            }
        }
        return null;
...
${messagePending}`
        ...
        //cek saldo downline
            ...
            // req query list saldo downline mk0001
            manager.query(`SELECT idreseller, NAMARESELLER, saldo FROM avr.masterreseller where idupline='mk0001'`, function (err, rows, fields) {
                if (err) {
                  console.error(err);
                  return;
                }
                ...
                const formattedRows = rows
                  .map(row => {
                    const adjustedSaldo = Math.floor(row.saldo / 500000) * 500000;
                    const ldl = `${row.idreseller} - ${row.NAMARESELLER} = Rp${row.saldo.toLocaleString('id-ID')}`
                    const linkWA = `https://wa.me/${centerTarikMk}?text=T.${row.idreseller}.-${adjustedSaldo}.2288`
                    if (adjustedSaldo > 0) {
                      return `${ldl}
${linkWA}`;
                    } else {
